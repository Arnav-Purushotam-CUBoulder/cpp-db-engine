...
TEST_CASE("query","[query]") {
  ...
  SECTION("query-memorystore") {
    ...
    new_db::EncodedValue value2("second valuable value");
    std::string key3("third string");
    new_db::EncodedValue value3("third valuable value");
    std::string key4("fourth string");
    new_db::EncodedValue value4("fourth valuable value");
    ...
    new_db::BucketQuery bq(queryBucket);
  ...
  SECTION("query-filestore") {
    std::cout << "Creating DB" << std::endl;
    std::string dbname("myemptydb");
    std::unique_ptr<new_db::KeyValueStore> kvs = std::make_unique<new_dbext::FileKeyValueStore>("./new_db/" + dbname);
    std::unique_ptr<new_db::IDatabase> db(new_db::GroundUpDB::createEmptyDB(dbname,kvs));
