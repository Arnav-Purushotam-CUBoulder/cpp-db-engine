...
public:
  ...
  operator new_db::HashedValue() {
    new_db::HashedValue hv(m_storage);

    return std::move(hv);
  }

  operator new_db::EncodedValue() {
    new_db::EncodedValue ev(m_storage);
...
template <typename T>
void check(const std::unique_ptr<new_db::IDatabase> &db, T v) {
  std::string k("testkey");
  new_db::EncodedValue ev(v);
  db->setKeyValue(k, std::move(ev));
  new_db::EncodedValue evr = db->getKeyValue(k);
  REQUIRE(ev.hasValue());
...
TEST_CASE("datatypes-container-memory", "[datatypes][container][memory]") {
  ...
  SECTION("datatypes-container-memory") {
    ...
    new_db::Bytes bytes; // aka std::vector<std::byte>
...
TEST_CASE("datatypes-customtypes-memory", "[.][datatypes][customtypes][memory]") {
  ...
  SECTION("datatypes-customtypes-memory") {
    ...
    // new_db::HashedValue hv = mct; // conversion operator
    // new_db::EncodedValue ev(hv); // conversion ctor

    new_db::EncodedValue ev = static_cast<new_db::EncodedValue>(mct); // conversion operator
