...
private:
  bool m_has_value;
  Type m_type; // internal new_db type identifier
...
{
    template<>
    struct hash<new_db::EncodedValue>
    {
        size_t operator()(const new_db::EncodedValue& v) const
        {
          std::size_t hv = 0;
    ...
    template<>
    struct hash<new_db::HashedValue>
    {
        size_t operator()(const new_db::HashedValue& v) const
        {
          std::size_t hv = 0;
          for (auto& vpart : v.data()) {
            hv = hv ^ (std::to_integer<std::size_t>(vpart) << 1);
    ...
    template<typename T>
    struct hash<new_db::Key<T>>
    {
        size_t operator()(const new_db::Key<T>& v) const
        {
          std::size_t hv = 0;
          for (auto& vpart : v.data()) {
            hv = hv ^ (std::to_integer<std::size_t>(vpart) << 1);
